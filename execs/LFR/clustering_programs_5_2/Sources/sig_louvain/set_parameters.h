...
void general_program_statement(char * b) {
	...
	cout<<"USAGE: "<<b<<" -f [network.dat] -folder [my_folder] -r [R] -seed [SEED]"<<endl<<endl;
	...
	cout<<"\n\n\n";
	cout<<"OPTIONS"<<endl;	
	cout<<"\n  [-r R]:\t\t\tsets the number of runs (the run with the best modularity will be chosen). Default value is 1."<<endl;
	...
	cout<<"output files will be tp, tp1 and so on for each hierarchical level (starting from the bottom)."<<endl;
...
Parameters::Parameters() {
	...
	Or=1;													// this is the number of global runs in the gather function		(first level)
	...
	command_flags.insert(make_pair("-folder", 16));
