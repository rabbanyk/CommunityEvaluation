...
public:
	...
	void set_rewired_matrix(int_matrix & ten, DI & labels);
	...
	void set_graph(deque<deque<int> > & link_per_node, deque<deque<double> > & weights_per_node, deque<int> & label_rows);
	...
	int set_upper_network(map<int, map<int, double > > & neigh_weight_f, int_matrix & P);
...
void static_network::set_rewired_matrix(int_matrix & ten, DI & labels) {
	
	
	// labels will be consecutive
...
void static_network::set_graph(deque<deque<int> > & link_per_node, deque<deque<double> > & weights_per_node, deque<int> & label_rows) {
...
int static_network::draw(string file_name) {
	...
	return draw(file_name, true);

}

int static_network::draw_consecutive(string file_name1, string file_name2) {
	
	return draw_consecutive(file_name1, file_name2, true);
}
...
void static_network::rewiring(deque<set<int> >& en) {
	...
	for(UI i=0; i<en.size(); i++) for(set<int>::iterator its=en[i].begin(); its!=en[i].end(); its++) if(int(i)<*its)
	...
	for(UI kk=0; kk<10; kk++) for(UI i=0; i<Es.size(); i++) {
...
void static_network::draw_pajek(map<int, double> & lab_x, map<int, double> & lab_y, string file_name, deque<deque<int> >  M, double scale_factor) {
	...
	for(UI i=0; i<M.size(); i++)
		for(UI j=0; j<M[i].size(); j++)
			node_member[M[i][j]].insert(i);
	//printm(node_member);
	
	
	for(UI i=0; i<node_member.size(); i++) {
...
int static_network::compute_betweeness_single_source(int source, map<pair<int, int>, double> & tot_edge_bw) {
	...
	while(true) {
		...
		for(UI i=0; i<present_shell.size(); i++)
...
int static_network::propagate_distances(deque<int> & new_shell, set<int> & already_gone, deque<pair<int, int> > & distances_node, int shell, deque<double> & ML, int & reached, int step) {
	...
	for(UI i=0; i<new_shell.size(); i++) for(int j=0; j<vertices[new_shell[i]]->links->size(); j++) {
...
int static_network::distances_from_i(int node, double & average, deque<double> & ML, int step) {
	...
	for(UI i=0; i<distances_node.size(); i++) {
	...
	for(int i=maxd+1; i<int(ML.size()); i++)
		ML[i]+=dim;
...
double static_network::monte_carlo_asp() {
	...
	for(int i=0; i<1000; i++) {
		...
		deque<double> ML;
		distances_from_i(irand(dim-1), average, ML, irand(dim-1));
...
double static_network::draw_gnuplot(map<int, double> & lab_x, map<int, double> & lab_y, ostream & goo, bool internal, deque<deque<int> >  M, double width) {
	...
	char b[1000];
	for (UI i=0; i<vertices.size(); i++) for (int j=0; j<vertices[i]->links->size(); j++) if(vertices[i]->id_num <= vertices[vertices[i]->links->l[j]]->id_num) {
...
int static_network::set_upper_network(map<int, map<int, double > > & neigh_weight_f, int_matrix & P) {
	...
	map<int, map<int, double> >  neigh_weight_s;

	RANGE_loop(i, P) {
				
		map<int, double> neigh_weight;
		neigh_weight_s.insert(make_pair(i, neigh_weight));
		neigh_weight_f.insert(make_pair(i, neigh_weight));
	}
	
	
	
	DI memberships(dim);
	RANGE_loop(i, P) RANGE_loop(j, P[i]) memberships[P[i][j]]=i;
	...
	for(int i=0; i<dim; i++) {
		...
		for(int j=0; j<vertices[i]->links->size(); j++) {
			...
			if(memberships[i]!=memberships[neigh])
				int_histogram(memberships[neigh], neigh_weight_s[memberships[i]], vertices[i]->links->w[j]);
		}
	...
	for(map<int, map<int, double > >:: iterator itm= neigh_weight_s.begin(); itm!= neigh_weight_s.end(); itm++) {
		
		for(map<int, double >::iterator itm_ = itm->second.begin(); itm_!=itm->second.end(); itm_++) if(itm_->first < itm->first) {
			neigh_weight_f[itm->first].insert(make_pair(itm_->first, itm_->second));
			neigh_weight_f[itm_->first].insert(make_pair(itm->first, itm_->second));
