...
void TAGMFast::NeighborComInit(const int InitComs) {
  ...
  TAGMFastUtil::GetNIdPhiV<PUNGraph>(G, NIdPhiV);
  NeighborComInit(NIdPhiV, InitComs);
}

void TAGMFast::NeighborComInit(TFltIntPrV& NIdPhiV, const int InitComs) {
  //initialize with best neighborhood communities (Gleich et.al. KDD'12)
  NIdPhiV.Sort(true);
  F.Gen(G->GetNodes());
  SumFV.Gen(InitComs);
...
void TAGMFast::SetCmtyVV(const TVec<TIntV>& CmtyVV) {
  ...
  for (int c = 0; c < CmtyVV.Len(); c++) {
    for (int u = 0; u < CmtyVV[c].Len(); u++) {
      int UID = NIDIdxH.GetDat(CmtyVV[c][u]);
      if (G->IsNode(UID)) { 
        AddCom(UID, c, 1.0);
      }
    }
...
void TAGMFast::GetCmtyVVUnSorted(TVec<TIntV>& CmtyVV) {
  GetCmtyVVUnSorted(CmtyVV, sqrt(2.0 * (double) G->GetEdges() / G->GetNodes() / G->GetNodes()), 3);
}

void TAGMFast::GetCmtyVVUnSorted(TVec<TIntV>& CmtyVV, const double Thres, const int MinSz) {
  CmtyVV.Gen(NumComs, 0);
#pragma omp parallel for schedule(static, 1)
  for (int c = 0; c < NumComs; c++) {
    TIntV CmtyV;
    for (int u = 0; u < G->GetNodes(); u++) {
      if (GetCom(u, c) > Thres) { CmtyV.Add(NIDV[u]); }
    }
#pragma omp critical(lock1) 
    {
      if (CmtyV.Len() >= MinSz) { CmtyVV.Add(CmtyV); }
...
int TAGMFast::FindComsByCV(const int NumThreads, const int MaxComs, const int MinComs, const int DivComs, const TStr OutFNm, const double StepAlpha, const double StepBeta) {
    ...
    return FindComsByCV(ComsV, 0.1, NumThreads, OutFNm, StepAlpha, StepBeta);
}
...
int TAGMFast::FindComsByCV(TIntV& ComsV, const double HOFrac, const int NumThreads, const TStr PlotLFNm, const double StepAlpha, const double StepBeta) {
  ...
  TFltIntPrV NIdPhiV;
  TAGMFastUtil::GetNIdPhiV<PUNGraph>(G, NIdPhiV);
  if (G->GetEdges() > 50) { //if edges are many enough, use CV
...
#pragma omp parallel for
    ...
    if (! PlotNm.Empty()) {
      printf("\n%d iterations [%s] %lu secs", iter * ChunkSize * ChunkNum, ExeTm.GetTmStr(), TSecTm::GetCurTm().GetAbsSecs() - StartTm);
    ...
    if ((iter - PrevIter) * ChunkSize * ChunkNum >= G->GetNodes()) {
      ...
      printf("\n%d iterations, Likelihood: %f, Diff: %f [%lu secs]", iter, CurL,  CurL - PrevL, time(NULL) - InitTime);
       fflush(stdout);
  ...
  if (! PlotNm.Empty()) {
    ...
    printf("\nMLE completed with %d iterations (%lu secs)", iter, time(NULL) - InitTime);
    fflush(stdout);
  }
  return iter;
}
