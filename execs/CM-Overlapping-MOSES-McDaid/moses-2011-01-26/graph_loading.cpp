...
const char *readEdge(const char *cur, int &l, int &r) throw (readEdgeInvalidLineInDataException) {
	...
	if(errno==EINVAL) { cout<< "1"; throw(readEdgeInvalidLineInDataException()); }

	while(!isdigit(*endptr)) endptr++; // advance to the start of the second name

	errno=0;
	r=strtol(endptr, &endptr, 10);
	if(errno==EINVAL) { cout<<"2"; throw(readEdgeInvalidLineInDataException()); }
...
const char *readEdge(const char *cur, string &l, string &r) throw (readEdgeInvalidLineInDataException) {
	...
	if(endptr == cur) { cout<<"3"; throw(readEdgeInvalidLineInDataException()); }
	...
	if(endptr == cur) {cout<<"4"; throw(readEdgeInvalidLineInDataException()); }
	cur=endptr;

	while(*endptr == '_' || *endptr == '-' || isalpha(*endptr) || isdigit(*endptr)) {
		endptr++;
	}
	if(endptr == cur) {cout<<"5"; throw(readEdgeInvalidLineInDataException()); }
