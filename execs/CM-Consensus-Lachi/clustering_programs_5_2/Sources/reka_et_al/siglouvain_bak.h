...
void siglouvain::module_initializing() {
	...
	for(int i=0; i<dim; i++) {
		...
		L_module newm(vertices[i]->stub_number);
...
void siglouvain::unweighted_favorite_of(const int & node, int & fi, int & kp, int & kop) {
...
void siglouvain::weighted_favorite_of(const int & node, int & fi, int & kp, int & kop) {
...
inline void siglouvain::update_modules(const int & i, const int & fi, const int & kp, const int & kop) {
...
void siglouvain::single_pass_unweighted() {
...
int siglouvain::collect_raw_groups_once(deque<deque<int> > & P) {
